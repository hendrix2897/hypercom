...
program. It is, in principle, very much like hypercom (direct fork) but
...
For a description of hypercom's operation, its command line options,
and usage examples, see the manual page included in the source
distribution as "hypercom.1", and also html-ized as "hypercom.1.html".

People who have contributed to hypercom, by offering feature
...
Change into hypercom's source directory and say:

    make

This will be enough to compile hypercom for most modern Unix-like
...
kilobytes. Then you can copy the hypercom binary, as well as the
man-page, to wherever you put your binaries and man-pages. For
example:


    cp hypercom ~/bin
    cp hypercom.1 ~/man/man1

Again, this is not strictly necessary. You can run hypercom and read
its man-page directly from the source directory.

If something goes wrong and hypercom can't compile cleanly, or if it's
...
You can start hypercom with its default option values (default serial
port settings) like this:

    hypercom /dev/ttyS0

If you have not installed the hypercom binary to a suitable place, then
...
port's device node. To overcome this you can run hypercom as root:

    sudo hypercom /dev/ttyS0
...
desired values using hypercom's command line options. For example, to
set the baud-rate to 115200bps (the default is 9600bps), and enable
hardware flow-control (RTS/CTS handshake) you can say:

    hypercom -b 115200 -f h /dev/ttyS0

or:

    hypercom --baud 115200 --flow h /dev/ttyS0

To see all available options run hypercom like this:

    hypercom --help

Once hypercom starts, it initializes the serial port and prints the
message:
...
then nothing that you type in hypercom will appear on your
terminal. This is normal.

To exit hypercom you have to type:
...
hypercom that the next character typed is to be interpreted as a
command to hypercom itself (in this case the exit command) and not to
be sent-down to the serial port. There are several other commands
(other than `C-a`, `C-x`), all prefixed by `C-a`.

Next you should take a look at the very detailed hypercom manual
page. It can be accessed like this (assuming you are inside the
hypercom distribution source directory):
...
Thanks for using hypercom

## Custom Bash completion

Starting with release 3.2, hypercom includes support for custom
...
1. If you are running a relatively modern Debian or Ubuntu or other
   ...
   the hypercom source directory, something like this will do it:

       sudo cp ./bash_completion/hypercom /etc/bash_completion.d/

   This will enable custom completion support for hypercom, globaly
   (for all Bash-shell users).

   *NOTICE:* If you have another version of hypercom already installed,
   there may already be a `hypercom` completion script in
...
2. If you want to automatically enable support *only for the current
   ...
   Copy the hypercom completion script to the directory you
   created. Assuming you are inside the hypercom source directory:

       cp ./bash_completion/hypercom ~/.bash_completion.d
...
You can use *hypercom* to patch-together a very simple, *very
...
simple as having the users say (from their remote workstations):
    ...
    $ ssh -t user@termbox hypercom -b 115200 /dev/ttyS0

Or make a convenient script/alias for this. Remember the `-t` switch
which instructs ssh to create a pseudo-tty, otherwise hypercom won't
work.

What if you *don't* want to give users shell-access to termbox? Then
you can use hypercom in a setup like the one described below. Just
...
Copy the hypercom binary in `~termbox/bin` (if you don't have it
globally installed):

    $ cp /path/to/hypercom ./bin
...
this:
    ...
    $ cat ./bin/ttyS0
    #!/bin/sh
    exec /home/termbox/bin/hypercom \
      --send-cmd '' \
      --receive-cmd '' \
      -b 115200 \
      /dev/ttyS0
...
`~termbox/bin` should look like this:
    ...
    $ ls -l ./bin
    -rwxrwxr-x 1 termbox termbox 102128 Aug 29 13:56 hypercom*
...
for the _ttyS0_ account, since hypercom will not read or write any
...
- If the default port settings you specified as command-line arguments
  to hypercom in `~termbox/bin/ttySx` do not match the settings of the
  device connected to the port, the user can easily change them from
  within hypercom, using hypercom commands.

- If a second user tries to remotely access the same port, at the same
  time, hypercom won't let him (hypercom will find the port locked and
  exit).

- In the example `~termbox/bin/ttySx` scripts we have completely
  disabled the send- and receive-file hypercom commands. This
  guarantees that hypercom won't execute any external commands. If you
...
that the underlying driver can handle this. Since release 2.0, hypercom
can be compiled with custom baudrate support for some systems. Since
release 3.1 hypercom is compiled with support enabled *by default* on
...
support to be *enabled* by compiling hypercom like this:
...
When hypercom is compiled with custom baudrate support *on Linux*, it
...
(though they should). In order to be able to use hypercom even in this
case, and without recompiling it, you can disable the custom baudrate
support at runtime, and force hypercom to use the "old" ioctls. To do
this (starting with release 3.2) just define the environment variable
`NO_CUSTOM_BAUD` before running hypercom. Something like this:

    NO_CUSTOM_BAUD=1 hypercom ...

This only applies to Linux, and to hypercom binaries that have been
